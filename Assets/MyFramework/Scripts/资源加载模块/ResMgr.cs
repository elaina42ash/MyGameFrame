...
public class ResMgr : BaseManager<ResMgr>
{
    ...
    /// <param name="callBack"> 加载结束后的回到函数 当异步加载资源结束后才会调用 </param>
    ...
    private IEnumerator ReallyLoadAsync<T>(string path) where T : UnityEngine.Object
    {
        ...
        // 等到资源加载结束后 才会继续执行 yield return 后面的代码
        yield return rq;
        ...
        if (resDic.ContainsKey(resName))
        {
            ...
            {
                UnloadAsset<T>(path,null,resInfo.isDel,false);
            }
            else
            {
                // 将加载完成的资源传递出去
                resInfo.callBack?.Invoke(resInfo.asset);
                // 加载完毕后 这些引用就可以清空 避免引用的占用 可能带来的潜在的内存问题
    ...
    /// <param name="callBack"> 加载结束后的回到函数 当异步加载资源结束后才会调用 </param>
    [Obsolete("注意：建议使用泛型加载方式，如果实在要用Type加载，一定不能喝泛型加载混用去加载同类型同名资源")]
    ...
    private IEnumerator ReallyLoadAsync(string path, Type type)
    {
        ...
        if (resDic.ContainsKey(resName))
        {
            ...
            if (resInfo.refCount == 0)
            {
                UnloadAsset(path, type,null,resInfo.isDel,false);
            }
            else
            {
                // 将加载完成的资源传递出去
    ...
    public void UnloadAsset<T>(string path, UnityAction<T> callBack = null, bool isDel = false,bool isSub = true)
    {
        string resName = path + "_" + typeof(T).Name;
        // 判断是否存在对应资源
        if (resDic.ContainsKey(resName))
    ...
    public void UnloadAsset(string path, Type type, UnityAction<UnityEngine.Object> callBack = null, bool isDel = false, bool isSub = true)
    {
        string resName = path + "_" + type.Name;
        // 判断是否存在对应资源
        if (resDic.ContainsKey(resName))
