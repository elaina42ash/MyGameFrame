...
public class TimerMgr : Singleton<TimerMgr>
{
    ...
    /// 创建单个计时器
    /// </summary>
    /// <param name="allTisRealTimeime"> 如果是true不受Time.timeScale影响 </param>
    /// <param name="allTime"> 总的时间 毫秒 1s = 1000ms </param>
    /// <param name="overCallBack"> 总时间结束回调 </param>
    /// <param name="intercalTime"> 间隔计时时间 毫秒 1s = 1000ms </param>
    /// <param name="callBack"> 间隔计时时间结束 </param>
    /// <returns> 返回唯一ID 用于外部控制对应计时器 </returns>
    public int CreateTimer(bool isRealTime, int allTime, UnityAction overCallBack, int intercalTime = 0, UnityAction callBack = null)
    {
        // 构建唯一id
        int keyID = ++TIMER_KEY;
        // 从缓存池取出对应的计时器
        ...
        // 初始化数据
        timerItem.InitInfo(keyID, allTime, overCallBack, intercalTime, callBack);
        // 记录到字典中 进行数据更新
        if (isRealTime)
