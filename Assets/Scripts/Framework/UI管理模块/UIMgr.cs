...
public class UIMgr : BaseManager<UIMgr>
{
    ...
    private UIMgr()
    {
        // 动态创建唯一的 Canvas 和 EventSystem（摄像机）
        ...
        // 动态创建EventSystem
        uiEventSystem = GameObject.Instantiate(ResMgr.Instance.Load<GameObject>("UI/EventSystem"))
    ...
    public void ShowPanel<T>(E_UILayer layer = E_UILayer.Middle, UnityAction<T> callBack = null, bool isSync = false) where T : BasePanel
    {
        ...
        if (panelDic.ContainsKey(panelName))
        {
            ...
            if (panelInfo.panel == null)
            {
                ...
                // 如果正在异步加载 应该等它加载完毕 只需要记录回调函数 加载完成后去调用即可
                if (callBack != null)
